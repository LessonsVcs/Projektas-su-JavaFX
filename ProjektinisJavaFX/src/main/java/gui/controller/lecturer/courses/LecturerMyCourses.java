...
public class LecturerMyCourses {
    ...
    private void openEditCourseWindow() {
        try {
            ...
            secondStage.initOwner(getPrimaryStage().getScene().getWindow());
            secondStage.setScene(secondWindow);
            viewManager.showLecturerEditCourse(secondStage, usernameToPass, course.getID());
            secondStage.showAndWait();
            updateTable();
        } catch (Exception e) {
    ...
    void showSelectedCourse(ActionEvent event) {
        if (event.getSource() == showCourse) {
            try {
                Course course = courseTable.getSelectionModel().getSelectedItem();
                viewManager.showLecturerShowMyCourse(course.getID(), usernameToPass, course.getName());
            } catch (Exception e) {
            }
        }
    }
